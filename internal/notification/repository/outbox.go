...
func (repo *Repository) InsertOutboxEvent(ctx context.Context, event *outboxService.OutboxEvent) error {
...
func (repo *Repository) InsertOutboxEvents(ctx context.Context, events []*outboxService.OutboxEvent) error {
	const op = "repository.InsertOutboxEvents"

	if len(events) == 0 {
		return nil
	}

	query := `INSERT INTO notification_outbox_events (event_id, event_type, payload, aggregate_type, aggregate_id, created_at) VALUES `
	...
	for _, e := range events {
		...
		args = append(args, e.EventID, e.EventType, e.Payload, e.AggregateType, e.AggregateID, e.CreatedAt)
	}
